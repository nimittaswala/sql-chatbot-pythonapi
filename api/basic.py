...
# Base directory of this file: /api
BASE_DIR = Path(__file__).resolve().parent

# Load local env if present
load_dotenv(BASE_DIR / "Config.env")

DEFAULT_SCHEMA_PATH = str(BASE_DIR / "schema_tree.json")
DEFAULT_KEYWORDS_PATH = str(BASE_DIR / "keyword_to_tables.json")
...
def resolve_path(path: str | Path | None, default_path: str) -> Path:
    """
    Resolve incoming path safely.
    - If missing/blank -> use default absolute path
...
def build_messages(slice_: Dict[str, Any], question: str, dialect: str) -> List[Dict[str, str]]:
    ...
    msgs: List[Dict[str, str]] = [
        {"role": "system", "content": SYSTEM_SQL.format(dialect=dialect)}
    ]
...
def ask_model(slice_: Dict[str, Any], question: str, dialect: str, model: str) -> str:
    ...
    resp = client.chat.completions.create(
        model=model,
        messages=build_messages(slice_, question, dialect),
    )
